...
class SpaceManager:
    ...
    def __init__(self, numObjects, sun=False, width:int=0, height:int=0) -> None:
        ...
        self.obj_pos = torch.tensor(self.obj_pos, dtype=torch.float32, device='cuda', requires_grad=False)

        self.obj_vel = np.random.randn(self.numObjects, 2) * .01
        self.obj_vel = torch.tensor(self.obj_vel, dtype=torch.float32, device='cuda', requires_grad=False)

        self.obj_mass = np.random.randn(self.numObjects) * 100 
        self.obj_mass = torch.tensor(self.obj_mass, dtype=torch.float32, device='cuda', requires_grad=False)

        # self.obj_mass[0] = 10_000
        self.mid = np.array([[self.width, self.height]]) / 2
        self.mid = torch.tensor(self.mid, dtype=torch.float32, device='cuda', requires_grad=False)
    ...
    def iterate2(self, screen):
        ...
        for i in range(self.numObjects):
            ...
            self.obj_vel[i] += torch.sum(force, axis=0) / self.obj_mass[i]
